...
def calculate_all_gini_coefficients(results_dir="."):
    ...
    all_results = []
    # CORRECTED: Include ALL 7 chains including TRG and TRD
    chains = ['IGH', 'IGK', 'IGL', 'TRA', 'TRB', 'TRG', 'TRD']
...
def create_diversity_plots_with_stats_pdf(combined_df):
    ...
    # CORRECTED: Include all T-cell chains (TRA, TRB, TRG, TRD)
    t_cells = combined_df[combined_df['chain'].isin(['TRA', 'TRB', 'TRG', 'TRD'])]
    if len(t_cells) > 0:
...
def create_diversity_plots_pdf(combined_df):
    ...
    # T-cell richness - CORRECTED to include all T-cell chains
    ax4 = axes[1, 1]
    t_cells = combined_df[combined_df['chain'].isin(['TRA', 'TRB', 'TRG', 'TRD'])]
    if len(t_cells) > 0:
        sns.boxplot(data=t_cells, x='chain', y='log_richness', hue='group', 
...
def create_gini_plots_pdf(gini_df):
    ...
    # Main Gini analysis figure - CORRECTED to include all 7 chains
    ...
    # 3. T-cell chains only - CORRECTED to include all T-cell chains
    ax3 = axes[0, 2]
    t_cells = gini_df[gini_df['chain'].isin(['TRA', 'TRB', 'TRG', 'TRD'])]
    if len(t_cells) > 0:
        t_chain_order = ['TRA', 'TRB', 'TRG', 'TRD']
        sns.boxplot(data=t_cells, x='chain', y='expansion_gini', hue='group', 
                    ax=ax3, palette=SEVERITY_PALETTE, order=t_chain_order)
        ax3.set_title('T-cell Clonal Expansion', fontsize=14, fontweight='bold')
        ax3.set_ylabel('Gini Coefficient')
        ax3.tick_params(axis='x', rotation=45)
    
    # 4. Heatmap - CORRECTED to include all chains
    ax4 = axes[1, 0]
    ...
    # Ensure all chains are present in the heatmap
    for chain in chain_order:
        if chain not in gini_pivot.columns:
            gini_pivot[chain] = np.nan
    
    gini_data = gini_pivot[chain_order].copy()
...
def create_bcell_tcell_comparison_pdf(gini_df):
    ...
    # Separate B and T cells - CORRECTED: T-cells now include TRG and TRD
    b_cells = gini_df[gini_df['chain'].isin(['IGH', 'IGK', 'IGL'])]
    t_cells = gini_df[gini_df['chain'].isin(['TRA', 'TRB', 'TRG', 'TRD'])]
...
def main():
    ...
    print("3. dengue_gini_coefficient_analysis.pdf - Gini coefficient analysis (now includes all 7 chains)")
    ...
    print("\n" + "="*70)
    print("NOTE: Now includes ALL immune receptor chains:")
    print("  - B-cells: IGH, IGK, IGL")
    print("  - T-cells: TRA, TRB, TRG, TRD")
    print("="*70)
